...
- **Quick links are now clickable.** Week-tab cards show their quick links as tappable chips (collapsed and open) that open in a new tab; the textarea stays for editing. Tip: a link like \`vscode://file/C:/dev/weekly-focus\` opens the repo folder in VS Code on the PC — browsers can't open plain folders, but app links like this work.
- **"Doable" tag + on-the-go mode.** Each Week card can be tagged **On the go** (just needs Claude/phone) or **Needs PC** (CC, design, cowork). The train button on Today tucks PC-only tasks into a "waiting for your PC" tray so commute-doable work stands out; overdue and ASAP tasks always stay visible. Per-device, like the mode toggle.
...
- **Structured quick-link editor.** Links are now added with Title + URL fields and listed as clickable chips with a remove button — no more "Label | url" text lines. Cards show the chips collapsed too.
- **Mobile sign-in fixed: 6-digit code.** The magic link opens the phone's browser, not the installed PWA — that's why mobile login failed. The sign-in email now also carries a 6-digit code you type inside the app (\`verifyOtp\`). Requires one Supabase change: Dashboard → Auth → Email Templates → Magic Link — make sure the template includes \`{{ .Token }}\` (e.g. "Your code: {{ .Token }}") alongside the link.
