...
(function () {
  ...
  /* ---- v59: dated Special sub-tasks belong on the calendar too.
     The Today screen reads x.when directly; the calendar used to read only
     entries["sched:..."], so every dated/overdue task was invisible here. ---- */
  function datedSubs(dateIso, mode) {
    ...
    activeItems("app").concat(activeItems("study")).filter(isSpecialItem).forEach(function (it) {
      activeSubs(subs(it.id)).forEach(function (x) {
        ...
        out.push({ it: it, x: x, v: v, w: w, od: !!(w && w.pastDue && !x.done), asap: !x.done && (x.urg || (v && v.asap)), soon: !!(v && v.soon && !x.done) });
      });
    });
    out.sort(function (a, b) {
      function rk(o) { return o.x.done ? 3 : o.od ? 0 : o.asap ? 1 : 2; }
      var ra = rk(a), rb = rk(b); if (ra !== rb) return ra - rb;
      return String(a.x.when) < String(b.x.when) ? -1 : 1;
    });
    return out;
  }
  /* every open dated sub-task, oldest first — drives the overdue jump banner */
  function overdueSubs(mode) {
    var out = [];
    activeItems("app").concat(activeItems("study")).filter(isSpecialItem).forEach(function (it) {
      visibleSubs(subs(it.id)).forEach(function (x) {
        if (x.done || !x.when) return;
        if (mode && !subModeOk(x.md, mode)) return;
        var v = whenView(x); if (!v || !v.w.pastDue) return;
        out.push({ iso: String(x.when).slice(0, 10), diff: v.w.diff });
      });
    });
  ...
  function hDcell(d, otherMonth) {
    var dIso = hIso(d), mode = getMode(), tasks = schedList(dIso, mode), dsubs = datedSubs(dIso, mode);
    var anyOd = dsubs.some(function (r) { return r.od; });
    var dots = dsubs.slice(0, 3).map(function (r) { return '<i class="' + (r.od ? "od" : r.asap ? "asap" : "sp") + '"></i>'; })
      .concat(tasks.slice(0, 3).map(function (s) { var k = kindOf(s.itemId); return "<i" + (k !== "app" ? ' class="' + k + '"' : "") + "></i>"; })).slice(0, 4).join("");
    return '<button class="dcell' + (dIso === hTodayIso() ? " today" : "") + (dIso === hSel ? " sel" : "") + (otherMonth ? " othermonth" : "") + (anyOd ? " hasod" : "") + '" data-hact="day" data-hkey="' + dIso + '">' +
  ...
  function renderCalScreen() {
    ...
    var mode = getMode(), tasks = schedList(hSel, mode), dsubs = datedSubs(hSel, mode);
    var nAll = tasks.length + dsubs.length;
    $("hDayN").textContent = nAll ? nAll + (nAll === 1 ? " task" : " tasks") : "free";
    var dayHtml = dsubs.map(function (r) { return hSubRow(r); }).join("") +
      tasks.map(function (s) { return hTaskRow({ key: s.key, itemId: s.itemId, done: s.done, act: "stoggle", removable: true }); }).join("");
    /* overdue tasks sit on dates you may not be looking at — offer a jump */
    var od = overdueSubs(mode), odHere = dsubs.filter(function (r) { return r.od; }).length;
    var odBanner = "";
    if (od.length && od.length > odHere) {
      var oldest = od[0], away = Math.max(1, -oldest.diff);
      odBanner = '<button class="od-banner cal-od" data-hact="odjump" data-hkey="' + oldest.iso + '">' + IC.flag +
        "<b>" + od.length + " overdue</b><span>oldest has waited " + away + (away === 1 ? " day" : " days") + " \u2014 tap to open that day</span></button>";
    }
    $("hDayRows").innerHTML = odBanner + (dayHtml || '<div class="hs-empty">No tasks on this day.</div>');
  }
  ...
  function wireHome() {
    ...
    document.addEventListener("click", function (e) {
      ...
      if (a === "odjump") { hSel = k; hAnchor = new Date(k + "T00:00:00"); renderCalScreen(); return; }
